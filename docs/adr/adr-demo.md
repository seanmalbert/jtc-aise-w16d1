...
We need to validate email addresses in our password reset endpoint before sending reset codes. Invalid emails cause delivery failures and poor user experience.

## Decision

We will use Pydantic's EmailStr type for email validation instead of writing custom regex patterns.
